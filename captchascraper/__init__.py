...
def post(url,apikey,data,**kwargs):
      return requests.post(f"http://api.scraperapi.com?api_key={apikey}&url=" + url,**kwargs)
...
def patch(url,apikey,data,**kwargse):
      return requests.patch(f"http://api.scraperapi.com?api_key={apikey}&url=" + url,**kwargs)
